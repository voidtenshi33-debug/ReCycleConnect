...
const PartCompatibilityInputSchema = z.object({
  partTitle: z.string().describe("The title or name of the spare part listing, e.g., 'Screen Assembly for Samsung Galaxy M31'."),
...
Task: Determine the compatibility between a spare part and a target device based on the user's descriptions.
Spare Part Title: "{{partTitle}}"
Spare Part Description: "{{partDescription}}"
Target Device: "{{userDeviceModel}}"

Analyze all available specifications like dimensions, connector types, model numbers, and release years to determine if the part is compatible with the target device.

Return your response in a JSON object with two keys:
- "compatibilityLevel": Choose one of 'High', 'Partial', or 'Incompatible'.
- "explanation": A brief, clear explanation for your reasoning, written for a non-expert.
`,
});
